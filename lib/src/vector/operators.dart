...
Vector<T> _resultVector<T>(
    int count, Vector<T> target, VectorFormat format, DataType<T> dataType) {
  if (target != null) {
  ...
  return Vector(dataType, count, format: format);
}
...
Vector<T> unaryOperator<T>(Vector<T> source, T Function(T value) operator,
  ...
  final result =
      _resultVector(source.count, target, format, dataType ?? source.dataType);
...
Vector<T> binaryOperator<T>(
  ...
  final result = _resultVector(
      sourceA.count, target, format, dataType ?? sourceA.dataType);
