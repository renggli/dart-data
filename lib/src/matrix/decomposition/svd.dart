...
class Svd {
  ...
  factory Svd(Matrix<double> matrix, [bool computeVectors = true]) {
  ...
  static List<double> _columnMajorValuesOf(Matrix<double> matrix) {
    ...
    for (var col = 0; col < matrix.colCount; col++) {
      for (var row = 0; row < matrix.rowCount; row++) {
        list[index++] = matrix.getUnchecked(row, col);
      }
    }
    return list;
  }
...
extension SvdExtension<T extends double> on Matrix<T> {
  /// Returns the Singular Value Decomposition of this [Matrix].
  Svd svd([bool computeVectors = true]) => Svd(this, computeVectors);

  /// Returns the rank, the effective numerical rank of this [Matrix].
  int rank() => Svd(this, false).rank;

  /// Calculates the nullity of the matrix.
  int nullity() => colCount - rank();

  /// Returns the condition, the ratio of largest to smallest singular value of
  /// this [Matrix].
  double conditionNumber() => Svd(this, false).conditionNumber;
}
